...
import rsa.randnum
import rsa.prime
from rsa.pem import load_pem, save_pem
...
class PrivateKey(AbstractKey):
    ...
    def _save_pkcs1_der(self):
        ...
        from rsa.asn1 import Encoder, Numbers
        ...
        encoder.write(self.q)
        encoder.write(self.exp1)
        encoder.write(self.exp2)
        encoder.write(self.coef)
        encoder.leave()
        asn_key = encoder.output()
    ...
    def export_key(self, name=None):
        """Saves a PKCS#1 PEM-encoded public key in a file.
        :return: None
        """
        key_pem = self._save_pkcs1_pem()
        if not name:
            name = 'RSAPriv.key'
        with open(name, 'wb') as pemfile:
            pemfile.write(key_pem)
...
__all__ = ['PrivateKey', 'PublicKey', 'newkeys']
