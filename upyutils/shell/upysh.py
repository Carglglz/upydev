...
def colorf(rdir, file):
    isdir = False
    try:
        isdir = os.stat("{}/{}".format(rdir, file))[0] & 0x4000
    except Exception:
        rdir = "{}/{}".format(os.getcwd(), rdir)
        isdir = os.stat("{}/{}".format(rdir, file))[0] & 0x4000
    if isdir:
        return "{}{}{}".format(ABLUE_bold, file, CEND)
    else:
        if not file.endswith(".py") and not file.endswith(".mpy"):
            return "{}{}{}".format(MAGENTA_bold, file, CEND)
        elif file.endswith(".mpy"):
            return "{}{}{}".format(CGREEN, file, CEND)
        return file
...
def print_table(
    ...
    if format_SH:
        ...
        _data = (
            "{}{}".format(
                colorf(rdir, file), " " * (wide - len(file)) + wrap(nf, cols, n)
            )
            for nf, file in enumerate(data)
        )
...
def _print_files(filelist, dir_name, show=True, hidden=False, gts=(40, 0)):
    if show:
        ...
        if filelist:
            print_table(filelist, wide=28, format_SH=True, gts=gts, rdir=dir_name)
                    ...
                    for exp_dir in expanded_dirs:
                        ...
                        if _files_in_dir:
                            if bydir:
                                ...
                                files_in_dir += [
                                    f"{exp_dir}/{file}" for file in _files_in_dir
                                ]
                        _files_in_dir = []  # reset
                else:
                    if bydir:
                        ...
                        files_in_dir += [
                            f"{dir}/{file}" for file in _os_match(pattrn, dir)
                        ]

            if files_in_dir:
                if bydir:
                    _print_files(
                        files_in_dir, dir_name, show=show, hidden=hidden, gts=gts
                    )
                    files_in_dir = []  # reset
        if files_in_dir:
            if not fullpath:
