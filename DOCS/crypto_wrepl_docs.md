...
####  In every session two new keys are generated (one by the host, and one by the device) so encryption and decryption use different keys.
...
* **The HANDSHAKE**:
  ...
  * Then the devices generates its session key and session IV using the same method above, and sends its token to the host, so the host can replicate the steps to generate the device session key and IV.
  ...
  So in the token there is no explicit info about any key, they bytes in the token are just indexes.
