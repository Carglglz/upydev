...
RAW_COMMANDS = {
    "toggle_led": "import time;led.on();time.sleep(1);led.off()",
    "run_test_code": "import test_code",
    "run_test_to_fail": "import test_to_fail",
}

DEVICE_RESULT = {
    "run_test_code": "test_code.RESULT",
    "run_test_to_fail": "test_to_fail.RESULT",
}

ASSERT_RESULT = {"run_test_code": True, "run_test_to_fail": True}
...
for command in list(RAW_COMMANDS.keys()):
    ...
    def add_command(cmd=command):
        TEST_NAME = cmd
        try:
            log.info(f"Running [{cmd}] test...")
            dev.wr_cmd(RAW_COMMANDS[cmd], follow=True)
            if cmd in ASSERT_RESULT.keys():
                RESULT = dev.wr_cmd(DEVICE_RESULT[cmd], silent=True, rtn_resp=True)
                # print(RESULT, ASSERT_RESULT[cmd])
                assert RESULT == ASSERT_RESULT[cmd], f"Test {cmd} FAILED"
            if cmd in RELOAD_SCRIPT.keys():
                dev.cmd(
                    f"import sys,gc;del(sys.modules['{RELOAD_SCRIPT[cmd]}']);"
                    f"gc.collect()"
                )
            do_pass(TEST_NAME)
            print("Test Result: ", end="")
    ...
    globals()[f"test_{command}"] = add_command
