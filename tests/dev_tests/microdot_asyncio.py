...
class Request(BaseRequest):
    ...
    async def _safe_readline(stream):
        ...
        while b"\n" not in line:
            ...
            )  # stream.s.readline() # for some reason unix blocks with await
            # stream.readline(), esp32 works fine.
            # if l:
            #     line += l
        # print(line)
...
class Microdot(BaseMicrodot):
    async def start_server(
        ...
        if aioctl.group():
            self.tasks = aioctl.group().tasks
        else:
            self.tasks = {}
        try:
            self.tasks["server"] = aioctl.Taskctl(
                None, self.server.task, "server", [], {}
            )
        except:
            pass
