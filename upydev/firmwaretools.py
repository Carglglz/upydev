...
for command, subcmd in FW_CMD_DICT_PARSER.items():
    _subparser = subparser_cmd.add_parser(
        command, help=subcmd["help"], description=subcmd["help"]
    )
    if subcmd["subcmd"]:
...
def firmwaretools_action(args, unkwargs, **kargs):
            ...
            for frfile in rest_args:
                if frfile not in os.listdir():
                    ...
                    try:
                        ...
                        if mpy_tool == 0:
                            print(
                                f"Process successful, bytecode in : "
    ...
    # FW
            ...
            if not args.b:  # Autodetect
                try:
                    ...
                    if dev.dev_platform == "pyboard":
                        machine = dev.cmd(
                            "import os;os.uname().machine", silent=True, rtn_resp=True
                        )
        ...
        if dt != "SerialDevice":
            print(
                f"indicate a serial port with -t option, {devname} is NOT a "
                "SerialDevice"
            )
            sys.exit()
        else:
            if "esp32" in fwfile:
                if args.i:
                    ...
                    if platform in fwfile:
                        print(
                            f"Firmware {fwfile} and {devname} device platform "
                        ...
                        sys.exit()
                print(
                    f"Flashing firmware {fwfile} with esptool.py "
                    f"to {devname} @ {args.t}..."
                )
                esptool_cmd_str = (
                    f"esptool.py --chip esp32 --port {args.t} "
                ...
                esptool_cmd_str = (
                    f"esptool.py --port {args.t} --baud 460800 "
                ...
                if flash_tool == "0":
                    ...
                    try:
                        ...
                        print(e)
                    print(
                        "Flashing firmware finished successfully, "
        ...
        if "esp32" in fwfile:
            ...
            if dt == "WebSocketDevice":
                ...
                dev = OTABleController(args.t, init=True, packet_size=512, debug=False)
                if dev.connected:
                    # ASSERT DFU MODE
                    ...
                    assert (
                        "DFU Control Point"
                        in dev.services_rsum["Device Firmware Update Service"]
                    ), "Missing DFU Control Point"
                    assert (
                        "DFU Packet"
                        in dev.services_rsum["Device Firmware Update Service"]
                    ), "Missing DFU Packet"
                    if fwfile:
                        assert fwfile.endswith(
                            ".bin"
                        ), "Incorrect file format, create the proper file"
