...
try:
    from pytest_benchmark.utils import report_online_progress
    from pytest_benchmark.utils import report_progress

    NUMBER_FMT = "{0:,.4f}" if sys.version_info[:2] > (2, 6) else "{0:.4f}"
    ALIGNED_NUMBER_FMT = "{0:>{1},.4f}{2:<{3}}" if sys.version_info[:2] > (
        2, 6) else "{0:>{1}.4f}{2:<{3}}"
    ...
    class TableResults(object):
        ...
        def display(self, tr, groups, progress_reporter=report_progress):
            ...
            for line, (group, benchmarks) in progress_reporter(groups, tr, "Computing stats ... group {pos}/{total}"):
                ...
                measure, unit, adjustment = self.scale_unit(unit=self.unit, benchmarks=benchmarks, best=best, worst=worst,
                                                            sort=self.sort)
                ops_unit, ops_adjustment = self.scale_unit(unit='operations', benchmarks=benchmarks, best=best, worst=worst,
                                                           sort=self.sort)
                labels = {
                    "name": f"Name ({measure} in {unit})",
                    "min": "Min",
                    "max": "Max",
                    "mean": "Mean",
                    "stddev": "StdDev",
                ...
                for bench in benchmarks:
                    ...
                    tr.write(bench["name"].ljust(widths["name"]),
                             red=has_error, invert=has_error)
                    for prop in self.columns:
                ...
                if self.histogram:
                    ...
                    output_file = make_histogram(
                        self.histogram, group, benchmarks, unit, adjustment)

                    self.logger.info("Generated histogram: {0}".format(
                        output_file), bold=True)
