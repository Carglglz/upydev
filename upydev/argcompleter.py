...
SET = dict(
    ...
    options={
        "-utc": dict(
            help='[utc] for "set ntptime" ' "command", required=False, nargs=1, type=int
        )
    },
...
UPTIME = dict(
    help=(
        "prints device's uptime since latest boot, "
...
UPDATE_UPYUTILS = dict(
    ...
    options={},
    alt_ops=os.listdir(os.path.join(UPYDEV_PATH, "upyutils_dir")),
)
ENABLE_SHELL = dict(
...
CONFIG = dict(
    ...
    desc="* needs config module\n* to set config --> [config]: " "[parameter]=[value]",
    subcmd=dict(
...
# DEBUGGING
PING = dict(
    help="ping the device to test if device is" " reachable, CTRL-C to stop.",
...
PROBE = dict(
    help="to test if a device is reachable",
    ...
    options={
        ...
        "-G": dict(help="internal flag for group mode", required=False, default=None),
        "-gg": dict(
            help="flag for global group",
            required=False,
            default=False,
            action="store_true",
        ),
        "-devs": dict(
            help="flag for filtering devs in global group", required=False, nargs="*"
        ),
    },
)

SCAN = dict(
    help="to scan for available devices, use a flag to filter for device type",
    desc="\ndefault: if no flag provided will do all three scans.",
    subcmd={},
    options={
        "-sr": dict(
            help="scan for SerialDevice",
            required=False,
            default=False,
            action="store_true",
        ),
        "-nt": dict(
            help="scan for WebSocketDevice",
            required=False,
            default=False,
            action="store_true",
        ),
        "-bl": dict(
            help="scan for BleDevice",
            required=False,
            default=False,
            action="store_true",
        ),
...
RUN = dict(
    help="run a script in device, CTRL-C to stop",
    desc="this calls 'import [script]' in device and reloads it at the end",
    subcmd=dict(help=("indicate a script to run"), metavar="script"),
    options={
        "-t": dict(help="device target address", required=True),
        "-p": dict(help="device password or baudrate", required=True),
        "-wss": dict(
        ...
        "-s": dict(
            help="indicate the path of the script if in external fs"
            " e.g. an sd card.",
            required=False,
        ),
    },
...
PLAY = dict(
    help="play custom tasks in ansible playbook style",
    desc="task must be yaml file with name, hosts, tasks, name, command\n" "structure",
    subcmd=dict(
        help=("indicate a task file to play."),
        metavar="task",
        choices=["add", "rm", "list"],
    ),
    options={
...
TIMEIT = dict(
    help="to measure execution time of a module/script",
    ...
    options={
        ...
        "-s": dict(
            help="indicate the path of the script if in external"
            " fs e.g. an sd card.",
            required=False,
        ),
    },
...
STREAM_TEST = dict(
    help="to test download speed (from device to host)",
    ...
    options={
        ...
        "-chunk_tx": dict(
            help="chunk size of data packets in kB to" " send",
            required=False,
            default=20,
            type=int,
        ),
        "-chunk_rx": dict(
            help="chunk size of data packets in kB to" " receive",
            required=False,
            default=32,
            type=int,
        ),
        "-total_size": dict(
            help="total size of data packets in MB",
            required=False,
            default=10,
            type=int,
        ),
...
SYSCTL = dict(
    help="to start/stop a script without following the output",
    desc="to follow initiate repl",
    mode=dict(
        help="indicate a mode {start,stop}", metavar="mode", choices=["start", "stop"]
    ),
    subcmd=dict(help="indicate a script to start/stop", metavar="script"),
    options={
        "-t": dict(help="device target address", required=True),
        "-p": dict(help="device password or baudrate", required=True),
        "-wss": dict(
...
LOG = dict(
    help="to log the output of a script running in device",
    desc="log levels (sys.stdout and file), run modes (normal, daemon) are"
    "available through following options",
    subcmd=dict(help=("indicate a script to run and log"), metavar="script"),
    options={
        "-t": dict(help="device target address", required=True),
        "-p": dict(help="device password or baudrate", required=True),
        "-wss": dict(
        ...
        "-dflev": dict(
            help="debug file mode level; default: error",
            default="error",
            choices=["debug", "info", "warning", "error", "critical"],
        ),
        "-dslev": dict(
            help="debug sys.stdout mode level; default: debug",
            default="debug",
            choices=["debug", "info", "warning", "error", "critical"],
        ),
        "-daemon": dict(
            help='enable "daemon mode", uses nohup so this '
        ...
        "-stopd": dict(
            help="To stop a log daemon script", default=False, action="store_true"
        ),
        "-F": dict(
            help="To follow a daemon log script file",
            action="store_true",
            default=False,
        ),
    },
...
PYTEST = dict(
    help="run tests on device with pytest (use pytest setup first)",
    subcmd=dict(
        help="indicate a test script to run, any optional " "arg is passed to pytest",
        default=[""],
        metavar="test",
        nargs="*",
    ),
...
CONFIG = dict(
    ...
    options={
        ...
        "-g": dict(
            help="save configuration in global path",
            required=False,
            default=False,
            action="store_true",
        ),
        "-gg": dict(
            help="save device configuration in global group",
            required=False,
            default=False,
            action="store_true",
        ),
        "-@": dict(help="specify a device name", required=False),
        "-zt": dict(help="zerotierone device configuration", required=False),
        "-sec": dict(
            help="introduce password with no echo",
            required=False,
            default=False,
            action="store_true",
        ),
...
CHECK = dict(
    help="to check device information",
    desc="shows current device information or specific device\n"
    "indicated with -@ option if it is stored in the global group.",
    subcmd={},
    options={
        "-@": dict(help="specify device/s name", required=False, nargs="+"),
        "-i": dict(
            help="if device is online/connected gets device info",
            required=False,
            default=False,
            action="store_true",
        ),
        "-g": dict(
            help="looks for configuration in global path",
            required=False,
            default=False,
            action="store_true",
        ),
        ...
        "-G": dict(help="specify a group, default: global group", required=False),
    },
)

SET = dict(
    help="to set current device configuration",
    subcmd={},
    options={
        "-@": dict(help="specify device name", required=False),
        "-g": dict(
            help="looks for configuration in global path",
            required=False,
            default=False,
...
REGISTER = dict(
    help="to register a device/group as a shell function so it is callable",
    subcmd=dict(help="alias for device/s or group", metavar="alias", nargs="*"),
    options={
        "-@": dict(help="specify device name", required=False, nargs="+"),
        "-gg": dict(
            help="register a group of devices",
            required=False,
            default=False,
            action="store_true",
        ),
        "-s": dict(help="specify a source file, default: ~/.profile", required=False),
        "-g": dict(
            help="looks for configuration in global path",
            required=False,
            default=False,
...
LSDEVS = dict(
    help="to see registered devices or groups",
    desc="this also defines a shell function with the same name in the source" " file",
    subcmd={},
    options={
        "-s": dict(help="specify a source file, default: ~/.profile", required=False),
        "-G": dict(help="specify a group, default: global group", required=False),
...
MKG = dict(
    ...
    options={
        ...
        "-devs": dict(
            help="device configuration [name] [target] " "[password]",
            required=False,
            nargs="+",
        ),
    },
...
GG = dict(
    help="to see global group of devices",
    subcmd={},
    options={
        "-g": dict(
            help="looks for configuration in global path",
...
SEE = dict(
    help="to see a group of devices",
    subcmd=dict(help="indicate a group name", metavar="group"),
    options={
        "-g": dict(
            help="looks for configuration in global path",
            required=False,
...
MGG = dict(
    help="manage a group of devices",
    desc="add/remove one or more devices to/from a group",
    subcmd=dict(help="group name", metavar="group", default="UPY_G", nargs="?"),
    options={
        "-g": dict(
            help="looks for configuration in global path",
            required=False,
        ...
        "-add": dict(help="add device/s name", required=False, nargs="*"),
        "-rm": dict(help="remove device/s name", required=False, nargs="*"),
        "-gg": dict(
            help="manage global group",
            required=False,
            default=False,
            action="store_true",
        ),
...
MKSG = dict(
    help="manage a subgroup of devices",
    desc="make group from another group with a subset of devices",
    subcmd=dict(help="group name", metavar="group", default="UPY_G", nargs="?"),
    sgroup=dict(help="subgroup name", metavar="subgroup"),
    options={
        "-g": dict(
            help="looks for configuration in global path",
            required=False,
        ...
        "-devs": dict(help="add device/s name", required=True, nargs="*"),
        "-gg": dict(
            help="manage global group",
            required=False,
            default=False,
...
# FW
MPYX = dict(
    help="freeze .py files using mpy-cross. (must be available in $PATH)",
    subcmd=dict(
        help="indicate a file/pattern to " "compile",
        default=[],
        metavar="file/pattern",
        nargs="+",
    ),
...
FW = dict(
    help="list or get available firmware from micropython.org",
    subcmd=dict(
        help=("{list, get, update}" "; default: list"),
        default=["list"],
        metavar="action",
        nargs="*",
    ),
    options={
        "-b": dict(help="to indicate device platform", required=False),
        "-n": dict(help="to indicate keyword for filter search", required=False),
        "-t": dict(help="device target address", required=True),
        "-p": dict(help="device password or baudrate", required=True),
        "-wss": dict(
            help="use WebSocket Secure",
...
FLASH = dict(
    help="to flash a firmware file using available serial tools "
    "(esptool.py, pydfu.py)",
    subcmd=dict(help=("indicate a firmware file to flash"), metavar="firmware file"),
    options={
        "-i": dict(
        ...
        "-t": dict(help="device target address", required=True),
        "-p": dict(help="device baudrate", required=True),
    },
)

OTA = dict(
    help="to flash a firmware file using OTA system (ota.py, otable.py)",
    subcmd=dict(help=("indicate a firmware file to flash"), metavar="firmware file"),
    options={
        "-i": dict(
            help="to check wether device platform and " "firmware file name match",
        ...
        "-t": dict(help="device target address", required=True),
        "-p": dict(help="device password", required=True),
        "-wss": dict(
            help="use WebSocket Secure",
            required=False,
            default=False,
...
RESET = dict(
    ...
    options={
        "-hr": dict(
            help="to do hard reset", required=False, default=False, action="store_true"
        )
    },
)
...
KBI = dict(help="to send KeyboardInterrupt to device", subcmd={}, options={})

UPYSH = dict(help="import upysh", subcmd={}, options={})
...
KG = dict(
    ...
    subcmd=dict(
        ...
        choices=["gen", "add", "export", "rotate", "dev", "host", "CA", "status"],
        default="gen",
        nargs="?",
    ),
    dst=dict(
...
SHELLREPLS = dict(
    ...
    options={
        ...
        "-rkey": dict(
            help="generate new password after exit " "(WebSocketDevices)",
            required=False,
            action="store_true",
        ),
        "-nem": dict(
            help="force no encryption mode" " (WebSocketDevices)",
            required=False,
            action="store_true",
        ),
    },
...
SHELL_CONFIG = dict(
    help="configure shell prompt colors",
    desc="see\nhttps://python-prompt-toolkit.readthedocs.io/en/master/"
    "pages/asking_for_input.html#colors\nfor color options",
    subcmd={},
    options={
        "--userpath": dict(
            help="user path color; default:" " ansimagenta bold",
            required=False,
            default="ansimagenta bold",
        ),
        "--username": dict(
            help="user name color; default:" " ansigreen bold",
            required=False,
            default="ansigreen bold",
        ),
        "--at": dict(
            help="@ color; default: ansigreen bold",
            required=False,
            default="ansigreen bold",
        ),
        "--colon": dict(
            help="colon color; default: white", required=False, default="#ffffff"
        ),
        "--pound": dict(
            help="pound color; default: ansiblue bold",
            required=False,
            default="ansiblue bold",
        ),
        "--host": dict(
            help="host color; default: ansigreen bold",
            required=False,
            default="ansigreen bold",
        ),
        "--path": dict(
            help="path color; default: ansiblue bold",
            required=False,
            default="ansiblue bold",
        ),
    },
...
SET_WSS = dict(
    help="toggle between WebSecREPL and WebREPL",
    subcmd={},
    options={
        "-t": dict(help="device target address", required=True),
        "-p": dict(help="device password", required=True),
...
JUPYTER = dict(
    help="MicroPython upydevice kernel for jupyter console, CTRL-D to exit",
    subcmd={},
    options={},
)

SHELLREPL_CMD_DICT_PARSER = {
    "shl": SHELLREPLS,
    "shl-config": SHELL_CONFIG,
    "set_wss": SET_WSS,
    "jupyterc": JUPYTER,
}
...
# FIO
PUT = dict(
    help="upload files to device",
    subcmd=dict(
        help="indicate a file/pattern/dir to " "upload",
        default=[],
        metavar="file/pattern/dir",
        nargs="+",
    ),
    options={
        "-dir": dict(help="path to upload to", required=False, default=""),
        "-rst": dict(
            help="to soft reset after upload",
            required=False,
            default=False,
            action="store_true",
        ),
...
GET = dict(
    help="download files from device",
    subcmd=dict(
        help="indicate a file/pattern/dir to " "download",
        default=[],
        metavar="file/pattern/dir",
        nargs="+",
    ),
    options={
        "-dir": dict(help="path to download from", required=False, default=""),
        "-d": dict(
            help="depth level search for pattrn", required=False, default=0, type=int
        ),
        "-fg": dict(
            help="switch off faster get method",
            required=False,
            default=True,
            action="store_false",
        ),
        "-b": dict(
            help="read buffer for faster get method",
            required=False,
            default=512,
            type=int,
        ),
...
DSYNC = dict(
    help="recursively sync a folder from/to device's filesystem",
    desc="* needs shasum.py in device",
    subcmd=dict(
        help="indicate a dir/pattern to " "sync",
        default=["."],
        metavar="dir/pattern",
        nargs="*",
    ),
    options={
        "-rf": dict(
            help="remove recursive force a dir or file deleted"
        ...
        "-t": dict(
            help="show tree of directory to sync",
            required=False,
            default=False,
            action="store_true",
        ),
        "-f": dict(
            help="force sync, no hash check",
            required=False,
            default=False,
            action="store_true",
        ),
        "-p": dict(
            help="show diff", required=False, default=False, action="store_true"
        ),
        "-n": dict(
            help="dry-run and save stash",
            required=False,
            default=False,
            action="store_true",
        ),
        "-i": dict(
            help="ignore file/dir or pattern", required=False, default=[], nargs="*"
        ),
    },
)
...
INSTALL = dict(
    help="install libraries or modules with upip to ./lib",
    subcmd=dict(help="indicate a lib/module to install", metavar="module"),
    options={},
)
