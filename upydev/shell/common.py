...
def parse_bash_profile():
    ...
    if ".bash_rc" in os.listdir(os.environ["HOME"]):
        ...
        with open(bash_rc, "r") as bf:
            ...
            bf_alias = [
                alias_line
                for alias_line in bf_content.splitlines()
    ...
    if ".profile" in os.listdir(os.environ["HOME"]):
        ...
        with open(profile, "r") as pf:
            ...
            pf_alias = [
                alias_line
                for alias_line in pf_content.splitlines()
                if alias_line.startswith("alias")
            ]
...
def print_table(
    ...
    if format_SH:
        ...
        data = [
            "{}{}{}{}".format(ABLUE_bold, val, CEND, " " * (wide - len(val)))
            if "." not in val
            else val
            for val in data
        ]
        data = [
            "{}{}{}{}".format(MAGENTA_bold, val, CEND, " " * (wide - len(val)))
            if ".py" not in val and ".mpy" not in val and "." in val
            else val
            for val in data
        ]
        data = [
            "{}{}{}{}".format(CGREEN, val, CEND, " " * (wide - len(val)))
            if ".mpy" in val
            else val
            for val in data
        ]
    if autocol_tab:
        data = [
            namefile if len(namefile) < wide else namefile + "\n" for namefile in data
        ]
    if autowide:
        wide_data = max([len(namefile) for namefile in data]) + 2
        if wide_data > wide:
...
def _os_match_dir(patt, path):
    ...
    return [
        ...
        if pattrn.match(dir) and os.stat(f"{path}/{dir}")[0] & 0x4000
    ]
                ...
                if "*" in dir_name:
                    ...
                    for exp_dir in expanded_dirs:
                        ...
                        if _files_in_dir:
                            if bydir:
                                _print_files(
                                    _files_in_dir, exp_dir, show=show, hidden=hidden
                                )
                            else:
                                files_in_dir += [
                                    f"{exp_dir.replace(os.getcwd(), '')}"
                                    f"/{file}".replace("./", "", 1)
...
class LTREE:
    ...
    def __call__(
        self, path=".", level=0, is_last=False, is_root=True, carrier="│", hidden=False
    ):
        if is_root:
        ...
        nf = len(
            [file for file in l if not os.stat("%s/%s" % (path, file))[0] & 0x4000]
        )
        nd = len(l) - nf
        ns_f, ns_d = 0, 0
        l.sort()
        ...
        for f in l:
            ...
            if st[0] & 0x4000:  # stat.S_IFDIR
                print(
                    self._treeindent(
                        level, f, last_file, is_last=is_last, carrier=carrier
                    )
                    + " \u001b[34;1m%s\033[0m" % f
                )
                ...
                for dir in resp.keys():
                    if not os.stat(dir)[0] & 0x4000:
                        if absp:
                            ...
                            print(
                                "{:9} {}".format(
                                    self.print_filesys_info(resp[dir]),
                                    dir.split("/")[-1],
                                )
                            )
                        ...
                        if dlev < max_dlev:
                            dlev += 1
                            self.__call__(
                                path=dir, dlev=dlev, max_dlev=max_dlev, hidden=hidden
                            )
                            dlev += -1
                        else:
                            if absp:
                                print(
                                    "{:9} \u001b[34;1m{}\033[0m".format(
                                        self.print_filesys_info(
                                            self.get_dir_size_recursive(dir)
                                        ),
                                        dir,
                                    )
                                )
    ...
    def get_dir_size_recursive(self, dir):
        return sum(
            [
                ...
                else self.get_dir_size_recursive(dir + "/" + f)
...
class CatFileIO:
    ...
    def do_pg_bar(
        self, index, wheel, nb_of_total, speed, time_e, loop_l, percentage, ett
    ):
        ...
        print(
            ...
            "{:>5} kB/s | {}/{} s".format(
                "█" * index
                + l_bloc
                + " " * ((self.bar_size + 1) - len("█" * index + l_bloc)),
                wheel[index % 4],
                int((percentage) * 100),
                nb_of_total,
                speed,
    ...
    def shapipe(self, data, std=True, **kargs):
        if std != "stderr" and data != "\n":
            ...
            sys.stdout.write("\033[A")
            print(
                f"{self._prog}: checking filesystem... "
    ...
    def shapipe_ble(self, data, std=True, **kargs):
        if std != "stderr" and data != "\n":
...
def get_dir_size_recursive(dir):
    return sum(
        [
            os.stat(dir + "/" + f)[6]
            if not os.stat(dir + "/" + f)[0] & 0x4000
            else get_dir_size_recursive(dir + "/" + f)
            for f in os.listdir(dir)
        ]
    )
