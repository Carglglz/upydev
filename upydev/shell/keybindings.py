...
def ShellKeyBindings(_flags, _dev, _shell, spc_cmds=[], kwdict=None):
    ...
    def _printpath():
        if flags.local_path["p"] == "":
            ...
            color_p = f"{_CG}{''.join(g_p[:-1])}{_CE}:{_BB}" f"{''.join(b_p)}{_CE}"
            print(color_p)
        else:
    ...
    def _autocomplete_config(buff_text):
        ...
        if is_config:
            ...
            if not show_param_opts:
                params_config_l = dev.wr_cmd(
                    "[conf for conf in os.listdir() " "if conf.endswith('_config.py')]",
                    silent=True,
                    rtn_resp=True,
                )

                _params_config = [param.split("_")[0] for param in params_config_l]
                _params_config += ["add"]
                if param_patt:
                    _params_config = [
                        param
                        for param in _params_config
                        if param.startswith(param_patt)
                    ]
            if show_param_opts:
                conf = param_patt
                _param_dir = dev.wr_cmd(
                    f"from {conf}_config"
                    f" import {conf.upper()};"
                    f"dir({conf.upper()})",
                    silent=True,
                        ...
                        if not glb:
                            if flags.shell_mode["S"]:
                                result = [
                                    val
                                    for val in output
                                    if val.startswith(buff_text.split("/")[-1])
                                ]
                                if len(result) > 1:
                                    comm_part = os.path.commonprefix(result)
    ...
    def _autocomplete_shell(event):
        if ")." not in event.app.current_buffer.document.text:
            buff_text = (
                event.app.current_buffer.document.text.replace("=", " ")
                .replace("(", " ")
                .split(" ")[-1]
            )
        else:
            buff_text = event.app.current_buffer.document.text.replace("=", " ").split(
                " "
            )[-1]
        rest = ""
        if "/" in buff_text:
            ...
            if buff_text.split("/")[-1] != "":
                rest = buff_text.split("/")[-1]
                cmd_ls_glb = (
                    f"[val for val in os.listdir('{dir_to_list}') "
                ...
                if dev.dev_class == "WebSocketDevice":
                    ...
                    def pprint_result():
                        ...
                        _printpath_cmd(last_cmd)
                        dev.wr_cmd(
                            f"print_table({cmd_ls_glb}, wide=28, format_SH=True,"
            ...
            if buff_text != "":
                cmd_ls_glb = (
                    f"[val for val in os.listdir() if "
            ...
            if isinstance(output, str):
                ...
                if rest != "":
                    ...
                    if not glb:
                        ...
                        result = [val for val in output if val.startswith(buff_text)]
                        if len(result) > 1:
                            comm_part = os.path.commonprefix(result)
                            if comm_part == buff_text:
                                if flags.shell_mode["S"]:
        ...
        try:
            ...
            if isinstance(buff_text, str):
                if "." in buff_text and not flags.shell_mode["S"]:
                    ...
                    if rest != "":
                        output = dev.wr_cmd(
                            f"[val for val in dir({root_text}) if "
                            f"val.startswith('{rest}')]",
                            silent=True,
                            rtn_resp=True,
                        )
                    ...
                    if import_cmd:
                        ...
                        if _imp and _from and len(fbuff_text) >= 3:
                            ...
                            if buff_text != "":
                                ...
                                flags.frozen_modules["SUB"] = [
                                    mod
                                    for mod in flags.frozen_modules["FM"]
                                    if mod.startswith(buff_text)
                                ]
                    try:
                        output = dev.wr_cmd(
                            cmd_ls_glb + ";gc.collect()", silent=True, rtn_resp=True
                        )
                    except KeyboardInterrupt:
                        pass
            else:
                ...
                try:
                    output = dev.wr_cmd(
                        f"dir({root_text});gc.collect()", silent=True, rtn_resp=True
                    )
                except KeyboardInterrupt:
                    pass
        except Exception:
    ...
    def paste_mode_exit(event):
        "PASTE MODE VIM EXEC, SOFT RESET IN REPL, EXIT IN SHELL"
        if flags.paste["p"]:
            app = get_app()
            p = app.key_processor
            inp = app.input
            ...
            if not flags.shell_mode["S"]:
                dev.reset()
                dev.wr_cmd(
                    "import gc;import os;from upysh import *;"
    ...
    def shif_tab(event):
        ...
        def autocomplete_sh_cmd():
            if flags.shell_mode["S"]:
                ...
                result = [
                    sh_cmd
                    for sh_cmd in shell_commands + custom_sh_cmd_kw + spc_cmds
                    if sh_cmd.startswith(buff_text)
                ]
                ...
                if len(buff_text.split()) > 0:
                    if "config" == buff_text.split()[0]:
                        ...
                        buff_text = ""
                if any(
                    [cmd in buff_text.split() for cmd in SHELL_CMD_DICT_PARSER.keys()]
                ):
