...
import argparse
import shlex
from upydev.shell.constants import (
    AUTHMODE_DICT,
    shell_commands,
    custom_sh_cmd_kw,
    shell_message,
...
class ShellCmds:
    ...
    def custom_sh_cmd(
        self, cmd, rest_args=None, args=None, topargs=None, ukw_args=None
    ):
    ...
    def escape_sh_cmd(self, cmd_inp):
        ...
        if cmd_inp.startswith("%") or cmd_inp.split()[0] not in self._shkw + [
            "-h",
            "-v",
        ]:
            if cmd_inp.split()[0] != "ping":
                try:
                    ...
                    # globals expand
                    _shell_cmd_str = [
                        cmd if "*" not in cmd else nglob(cmd) for cmd in shell_cmd_str
                    ]
                    shell_cmd_str = []
                    ...
                    if shell_cmd_str[0] in SHELL_ALIASES:
                        shell_cmd_str = (
                            SHELL_ALIASES[shell_cmd_str[0]].split() + shell_cmd_str[1:]
                        )
                    if shell_cmd_str[0] in SHELL_FUNCTIONS:
                        shell_cmd_str = (
                            SHELL_FUNCTIONS[shell_cmd_str[0]].split()
                            + shell_cmd_str[1:]
                        )
    ...
    def sh_cmd(self, cmd_inp):
        ...
        if any([pipecmd in unknown_args for pipecmd in self._pipe_flags]):
            ...
            if not isinstance(rest_args, list):
                rest_args = [rest_args]
            cmd_to_pipe = f"{command} {' '.join(rest_args)} " f"{' '.join(opt_args)}"
            # print(cmd_to_pipe)
            try:
                if pflag == ">":
                    wmode = "wb"
        ...
        if command == "mem":
            if rest_args == "info":
                ...
                mem = {
                    elem.strip().split(":")[0]: int(elem.strip().split(":")[1])
                    for elem in mem_info[4:].split(",")
                }
                ...
                print(
                    f"{'RAM':12}{total_mem_s:^12}{used_mem_s:^12}{free_mem_s:^12}"
                    f"{percent_mem:>8}"
                )
            else:
                if rest_args == "dump":
        ...
        if command == "df":
            ...
            print(
                f"{'Filesystem':12}{'Size':^12}{'Used':^12}{'Avail':^12}{'Use%':^12}"
                f"{'Mounted on':^12}"
            )
            print(
                f"{'Flash':12}{total_mem:^12}{used_mem:^12}{free_mem:^12}"
            ...
            for vfs in vfs_resp.keys():
                if vfs_resp[vfs] != resp:
                    ...
                    free_mem_sd = self.print_filesys_info(
                        size_info_sd[0] * size_info_sd[3]
                    )
                    ...
                    mounted = f"/{vfs}"
                    print(
                        f"{vfs:12}{total_mem_sd:^12}{used_mem_sd:^12}"
                        f"{free_mem_sd:^12}"
        ...
        if command == "du":
            if not rest_args:
                ...
                du_dir = rest_args
                self.send_cmd(
                    f"from upysh2 import du;du(path='./{du_dir}',"
        ...
        if command == "tree":
            if not rest_args:
                self.send_cmd(
                    "from upysh2 import tree;" f"tree(hidden={args.a});gc.collect()",
                    sh_silent=False,
                    follow=True,
                )
                ...
                tree_dir = rest_args
                self.send_cmd(
                    f"from upysh2 import tree;tree('{tree_dir}', "
                    f"hidden={args.a})"
                    f";gc.collect()",
                    sh_silent=False,
                    follow=True,
        ...
        if command == "ifconfig":
            ...
            if sta_isconnected:
                ifconf = self.send_cmd("network.WLAN(network.STA_IF).ifconfig()")
                if not args.t:
                    print(ifconf)
                else:
                    essid = self.send_cmd(
        ...
        if command == "net":
            if rest_args == "status":
                ...
                if rest_args == "on":
                    stat_on = self.send_cmd(
                        _CMDDICT_["NET_STAT_ON"], raise_devexcept=True
                    )
                    if stat_on:
                        print("Station Enabled")
                elif rest_args == "off":
                    stat_on = self.send_cmd(
                        _CMDDICT_["NET_STAT_OFF"] + "\r" * 5, raise_devexcept=True
                    )
                    if stat_on:
                        print("Station Disabled")
                    ...
                    if enable_sta:
                        ...
                        for net in scan:
                            ...
                            if len(ap_name) > 20:
                                ap_name = ap_name[:17] + "..."
                            bssid = ":".join(
                                [vals[i : i + 2] for i in range(0, len(vals), 2)]
                            )
                            print(
                                "┃{0:^20} ┃ {1:^25} ┃ {2:^10} ┃ {3:^15} ┃"
                                " {4:^15} ┃ {5:^10} ┃".format(
        ...
        if command == "ap":
            if rest_args == "status":
                ...
                if rest_args == "on":
                    ap_on = self.send_cmd(_CMDDICT_["AP_ON"], raise_devexcept=True)
                    if ap_on:
                    ...
                    ap_off = self.send_cmd(_CMDDICT_["AP_OFF"], raise_devexcept=True)
                    if ap_off:
                        print("Access Point Disabled")
                    ...
                    ap_scan_list = self.send_cmd(
                        _CMDDICT_["AP_SCAN"], raise_devexcept=True
                    )
                    if enable_sta:
        ...
        if command == "i2c":
            if rest_args == "config":
                ...
                if self.dev.dev_platform == "pyboard":
                    self.send_cmd(
                        _CMDDICT_["I2C_CONFIG_PYB"].format(scl, sda),
                        raise_devexcept=True,
                    )
                else:
                    self.send_cmd(
                        _CMDDICT_["I2C_CONFIG"].format(scl, sda), raise_devexcept=True
                    )

                print(f"I2C configured:\nSCL = Pin({scl}), SDA = Pin({sda})")
            elif rest_args == "scan":
                i2c_scan_list = self.send_cmd(
                    _CMDDICT_["I2C_SCAN"], raise_devexcept=True
                )
        ...
        if command == "set":
            ...
            if rest_args[0] == "rtc":
                ...
                if rest_args[1] == "localtime":
                    ...
                    self.send_cmd(
                        _CMDDICT_["SET_RTC_LT"].format(*datetime_final),
                        raise_devexcept=True,
                    )
        ...
        if command == "config":
            if not rest_args:
                ...
                dev_config = self.dev.wr_cmd(
                    "glob('*_config.py')", silent=True, rtn_resp=True
                )
                _params_config = [
                    param.split("_")[0].rsplit("/")[-1] for param in dev_config
                ]
                ...
                if rest_args[0] != "add":
                    if not rest_args[0].endswith(":"):  # check config
                        ...
                        dev_config = self.dev.wr_cmd(
                            f"glob(*{rest_args})", silent=True, rtn_resp=True
                        )
                        _params_config = [
                            (
                                param.split("_")[0].rsplit("/")[-1],
                                param.replace(".py", "").rsplit("/")[-1],
                            )
                            for param in dev_config
                        ]
                        params_config = {
                            param: self.dev.wr_cmd(
                        ...
                        for conf in params_config.keys():
                            ...
                            for param in param_in_config:
                                def_conf[param] = param_parser(
                                    params_config[conf], param
                                )
                            if args.y:
                                conf_str = "\n    ".join(
                                    [f"{k}: {v}" for k, v in def_conf.items()]
                                )
                                print(f"{conf}: \n    {conf_str}")
                            else:
                                conf_str = ", ".join(
                        ...
                        new_conf_str = (
                            f"from config.params import set_{param_option}"
                    ...
                    if len(rest_args) > 1:
                        ...
                        for nc in new_confs:
                            self.dev.wr_cmd(
                                f"from config import add_param; " f"add_param('{nc}')"
                            )
                        reload_cmd = (
                            "import sys;del(sys.modules['config.params']);"
                            "gc.collect()"
                        )
                        self.dev.wr_cmd(reload_cmd, silent=True)
                    else:
        ...
        if command == "shasum":
            if args.c:
                ...
                self.send_cmd(
                    _CMDDICT_["SHASUM_CHECK"].format(args.c),
                    sh_silent=False,
                    follow=True,
                    raise_devexcept=True,
                )
            ...
            if not rest_args:
                ...
                self.send_cmd(
                    _CMDDICT_["SHASUM"].format(files_to_hash),
                    sh_silent=False,
                    follow=True,
                )
        ...
        if command == "rm":
            if not rest_args:
                ...
                if args.d:
                    _rest_args = [
                        [("*/" * i) + patt for i in range(args.d)] for patt in rest_args
                    ]
                    rest_args = []
                ...
                if not args.rf:
                    ...
                    self.dev.wr_cmd(
                        f"rmrf(*glob(*{rest_args},dir_only={args.dd}))", follow=True
                    )
            return
        ...
        if command == "rmdir":
            if not rest_args:
                ...
                self.dev.wr_cmd(
                    f"rmdir(*glob(*{rest_args}, dir_only=True))", follow=True
                )
            return
        ...
        if command == "mv":
            ...
            dst = rest_args[1]
            self.dev.wr_cmd(
                f"import os;os.rename('{src}', '{dst}')", silent=False, follow=True
            )
        ...
        if command == "ls":
            ...
            try:
                ...
                term_size = (80, 80)
            self.send_cmd(
                _CMDDICT_["LS"].format(files_to_list, term_size, args.a),
                sh_silent=False,
                follow=True,
            )
            return
        ...
        if command == "cat":
            if not rest_args:
                ...
                self.send_cmd(
                    _CMDDICT_["CAT"].format(files_to_see), sh_silent=False, follow=True
                )
        ...
        if command == "head":
            if not rest_args:
                ...
                files_to_see = f"*{rest_args}"
                self.send_cmd(
                    _CMDDICT_["HEAD"].format(files_to_see, args.n),
                    sh_silent=False,
                    follow=True,
                )
        ...
        if command == "touch":
            ...
            if not rest_args:
                ...
                self.send_cmd(
                    _CMDDICT_["TOUCH"].format(files_to_create),
                    sh_silent=False,
                    follow=True,
                )
                ...
                try:
                    for script in rest_args:
                        if script.endswith(".py") or script.endswith(".mpy"):
                            ...
                            script_to_time = script.replace(".py", "").replace(
                                ".mpy", ""
                            )
                            inp = (
                                f"from time_it import tzero, tdiff, result;t_0 = "
        ...
        if command == "run":
            if not rest_args:
                ...
                if args.r:
                    ...
                    reload_cmd = f"import sys;{reload};" f"gc.collect()"
        ...
        if command == "docs":
            if rest_args:
                key = rest_args
                search = (
                    f"https://upydev.readthedocs.io/en/latest/search.html?q={key}&"
        ...
        if command == "mdocs":
            if rest_args:
                key = rest_args
                search = (
                    f"https://docs.micropython.org/en/latest/search.html?q={key}&"
                    "check_keywords=yes&area=default"
                )
                webbrowser.open(search)
            else:
        ...
        if command == "diff":
            ...
            if "*" in rest_args[0]:
                ...
                if dev_files:
                    local_files = shasum(
                        *[rest_args[0]], debug=False, rtn=True, size=True
                    )
                if local_files:
                    files_to_diff = [
                        (fts[0], fts[2]) for fts in dev_files if fts not in local_files
                    ]
            ...
            if nglob(file_to_edit):
                ...
                _file_to_edit = file_to_edit.replace(_file_to_edit, f"~{_file_to_edit}")
            ...
            if not args.s:
                diff_cmd = (
                    f"git diff --color-words --no-index "
        ...
        if command == "update_upyutils":
            ...
            files = nglob(*[os.path.join(SRCDIR, file) for file in rest_args])
            is_lib = self.dev.wr_cmd(
                "import os; 'lib' in os.listdir()", rtn_resp=True, silent=True
            )
            if not is_lib:
            ...
            if self.dev.dev_class == "SerialDevice":
                ...
                fileargs.s = "/lib"
                fileio = WebSocketFileIO(self.dev, args=fileargs, devname=self.dev_name)
                fileio.put_files(fileargs, self.dev_name)
        ...
        if command == "aioctl":
            ...
            if rest_args == "status":
                ...
                if sbcmd != "add":
                    if sbcmd == "follow":
                        self.send_cmd(
                            f"await aioctl.{sbcmd}('{_rest_args[0]}')",
                            sh_silent=False,
                            follow=True,
                        )
                    else:
                        self.send_cmd(
                            f"import aioctl;aioctl.{sbcmd}('{_rest_args[0]}')",
                            sh_silent=False,
                            follow=True,
                        )
                else:
                    ...
                    self.send_cmd(
                        f"import aioctl;aioctl.{sbcmd}({_rest_args})",
                        sh_silent=False,
                        follow=True,
                    )
            return
    ...
    def get_rprompt(self):
        ...
        if self.flags.mem_show_rp["show"]:
            if self.flags.mem_show_rp["percent"] < 85:
                return HTML(
                    '<aaa fg="ansiblack" bg="ansiwhite"> RAM </aaa><b><style '
