            ...
            if "esp32" in dev_p:
                if args.i:
                    ...
                    if self.dev.dev_platform in rest_args:
                        print(
                            f"Firmware {rest_args} and {self.dev_name} "
                ...
                esptool_cmd_str = (
                    f"esptool.py --port {self.dev.serial_port} "
                ...
                if args.i:
                    ...
                    if platform in rest_args or platform[:3] in rest_args:
                        ...
                        if fw_string == rest_args.split("-")[0]:
                            print(
                                f"Firmware {rest_args} and {devname} device platform "
    ...
    def mpycross(self, args, rest_args):
        ...
        for file in rest_args:
            ...
            try:
                ...
                if mpy_tool == 0:
                    print(
                        "Process successful, bytecode in : {}".format(
                            file.replace("py", "mpy")
                        )
                    )
                else:
        ...
        if "esp32" in fwfile:
            ...
            if args.i:
                ...
                if self.dev.dev_platform in fwfile:
                    print(
                        f"Firmware {fwfile} and {self.dev_name} device platform "
            ...
            if self.dev.dev_class == "WebSocketDevice":
                ...
                if not args.bg:
                    ...
                    print("Check OTA process with aioctl status as_ota*")
                    print("and reset when done")
                    time.sleep(1)
                ...
                time.sleep(2)
                dev = OTABleController(
                    self.dev.address, init=True, packet_size=512, debug=False
                )
                if dev.connected:
                    # ASSERT DFU MODE
                    try:
                        assert (
                            "Device Firmware Update Service" in dev.services_rsum
                        ), "DFU Mode not available"
                        fwu_services = dev.services_rsum[
                            "Device Firmware " "Update Service"
                        ]
                        assert "DFU Control Point" in fwu_services, (
                ...
                while True:
                    try:
                        ...
                        time.sleep(1)
                        print(
                            f"ota: waiting for {self.dev_name} to "
                            "be available again..."
                        )
                time.sleep(1)
                self.dev.wr_cmd("import gc;from upysh import *", silent=True)
