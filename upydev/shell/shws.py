...
WREPL = dict(help="enter WebREPL", subcmd={}, options={})
GETCERT = dict(help="get device's certificate if available", subcmd={}, options={})
JUPYTERC = dict(
    help="enter jupyter console with upydevice kernel", subcmd={}, options={}
)
...
PUT = dict(
    ...
    options={"-dir": dict(help="path to upload to", required=False, default="")},
)
...
DSYNC = dict(
    ...
    options={
        ...
        "-app": dict(
            help="apply stash", required=False, default=False, action="store_true"
        ),
        "-s": dict(
            help="show stash", required=False, default=False, action="store_true"
        ),
    },
)

DEBUG = dict(help="toggle debug mode for websocket debugging", subcmd={}, options={})
...
OTA = dict(
    help="to flash a firmware file using OTA system",
...
INSTALL = dict(
    help="install libraries or modules with mip to ./lib",
    subcmd=dict(help="indicate a lib/module to install", metavar="module"),
    options={},
)

SHELLWS_CMD_DICT_PARSER = {
    "repl": WREPL,
    "getcert": GETCERT,
    "jupyterc": JUPYTERC,
    "pytest": PYTEST,
...
class ShellWsCmds(ShellCmds):
    def __init__(self, *args, **kargs):
        ...
        self.dsyncio = ShDsyncIO(
            self.dev, self.dev_name, self.fileio, self.fileio, shell=self
        )
    ...
    def custom_sh_cmd(
        ...
        if cmd == "upy-config":
            if not self.dev.dev_platform:
                self.dev.dev_platform = self.dev.wr_cmd(
                    "import sys; sys.platform", silent=True, rtn_resp=True
                )
        ...
        if cmd == "install":
            ...
            self.dev.wr_cmd(f"import mip;mip.install('{rest_args}');True", follow=True)
