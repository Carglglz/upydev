...
class ShDsyncIO:
    ...
    def sr_get(self, args, src_name, sz, dst_name):
        if not args.fg:
            try:
                self.fileio.get((sz, src_name), dst_name, fullpath=True, psize=False)
            except KeyboardInterrupt:
            ...
            try:  # FAST GET
                ...
                cmd_gf = f"rcat('{src_name}', buff={args.b});" f"gc.collect()"
                if sz > 0:
    ...
    def ws_get(self, args, src_name, sz, dst_name):
        if not args.fg:
            ...
            ws = websocket(self.dev.ws.sock)
            try:
                get_file(ws, dst_name, src_name, sz)
            except KeyboardInterrupt:
                print("KeyboardInterrupt: get Operation Canceled")
                # flush ws and reset
                self.dev.flush()
            ...
            try:
                ...
                cmd_gf = (
                    f"rcat('{src_name}', buff={args.b},"
                    f" stream=wss_repl.client_swr);"
                    f"gc.collect()"
                )
                if sz > 0:
                    # self.fileio.ws_get_file(cmd_gf)
                    self.fileio.rs_get_file(cmd_gf, chunk=args.b)
                    self.fileio.save_file()
                else:
                    self.fileio.do_pg_bar(
                        self.fileio.bar_size,
                        self.fileio.wheel,
                        f"{0:.2f}/{0:.2f} KB",
                        "0",
                        0,
    ...
    def ble_get(self, args, src_name, sz, dst_name):
        if not args.fg:
            ...
            try:  # FAST GET
                self.fastfileio.init_get(dst_name, sz)
                cmd_gf = f"cat('{src_name}');" f"gc.collect()"
                self.dev.wr_cmd(
                    cmd_gf,
                    follow=True,
                    long_string=True,
    ...
    def read_file_tree(self, path="", tree={}):
        files = self.dev.cmd(
            f'import os; os.chdir("/"); [f for f in os.ilistdir("{path}")]',
            silent=True,
            rtn_resp=True,
        )
        for f_name, f_type, f_inode, f_size in files:
            new_path = f_name if path == "" else f"{path}/{f_name}"
            if f_type == 32768:  # if file
                tree[new_path] = f_size
            else:
                self.read_file_tree(f"{path}/{f_name}", tree)
                tree[new_path] = 0
        return tree
    ...
    def fileop(self, cmd, args, rest_args):
        if cmd == "put":
            ...
            if file_match:
                source = "/"
                file_match = [
                    (sz, file.replace(os.getcwd(), ".")) for sz, file in file_match
                ]
                if args.dir:
                    source = args.dir
        ...
        if cmd == "get":
            ...
            if args.fg:
                file_match = self.dev.cmd(
                    f"from nanoglob import glob; " f"glob(*{rest_args}, size=True)",
                    silent=True,
                    rtn_resp=True,
                )
            else:
                ...
                for patt in rest_args:
                    if "*" not in patt:  # not pattern searching
                        ...
                        pattrn = re.compile(
                            patt.replace(".", r"\.").replace("*", ".*") + "$"
                        )
                        for f in file_tree:
                            if not pattrn.match(f):
                                continue
                            file_match.append((file_tree[f], f))
            ...
            if file_match:
                ...
                if args.dir:
                    print(
                        f'get: {", ".join(rest_args)}: No matching files found in '
    ...
    def fsync(self, args, rest_args):
        if not args.d:
            ...
            local_hashlist = shasum(
                *rest_args, all=True, size=True, recursive=True, rtn=True, debug=False
            )
            # SEPARATE
            if local_hashlist:
                ...
                if not args.f:
                    ...
                    if isdir:
                        ...
                        if dev_hashlist:
                            ...
                            print("", end="\r")
                        dev_dir_match = [
                            dir for dir, sz, id in dev_hashlist if id == "dir"
                        ]
                        dev_file_match = [fh for fh in dev_hashlist if fh[-1] != "dir"]
            ...
            if args.n:
                self.stash.update(
                    dirs=dir_match,
                    files=file_match,
                    dev_dirs=dev_dir_match,
                    dev_files=dev_file_match,
                )

            dirs_to_make = [ldir for ldir in dir_match if ldir not in dev_dir_match]
                    ...
                    if self.dev.dev_class == "WebSocketDevice":
                        if len(f"mkdr(*{dirs_to_make})") > 250:
                            for i in range(0, len(dirs_to_make), 10):
                                self.dev.wr_cmd(
                                    f"mkdir(*{dirs_to_make[i:i+10]})", follow=True
                                )
                        else:
            ...
            if dev_files:
                files_to_sync = [
                    (fh[1], fh[0]) for fh in local_files if fh not in dev_files
                ]
            else:
                files_to_sync = [(fh[1], fh[0]) for fh in local_files]
            if args.i:
                _file_match = self.re_filt(args.i, [nm for sz, nm in files_to_sync])
                files_to_sync = [
                    (sz, nm) for sz, nm in files_to_sync if nm in _file_match
                ]
            ...
            if files_to_sync:
                ...
                _new_files = [
                    (sz, name)
                    for sz, name in files_to_sync
                    if name not in [dname for dname, sz, fh in dev_files]
                ]
                _modified_files = [
                    (sz, name)
                    for sz, name in files_to_sync
            ...
            if args.t:
                if isinstance(top_dir, list):
                    for dir in top_dir:
                        try:
                            self.dev.wr_cmd(
                                f"from upysh2 import tree;" f"tree('{dir}')",
                                follow=True,
                            )
                        except Exception:
                            pass
            ...
            if self.dev.dev_class == "BleDevice":
                shapipe = ff.shapipe_ble
            dev_hashlist = self.dev.wr_cmd(
                dev_hash_cmd, follow=True, rtn_resp=True, long_string=True, pipe=shapipe
            )
            dev_hashlist = ff._shafiles
            # SEPARATE
            if dev_hashlist:
                ff.end_sha()
                print("", end="\r")
                dev_dir_match = [dir for dir, sz, id in dev_hashlist if id == "dir"]
                dev_file_match = [fh for fh in dev_hashlist if fh[-1] != "dir"]
            ...
            dirs_to_make = []
            if dev_dir_match:
                dirs_to_make = [ddir for ddir in dev_dir_match if ddir not in dir_match]
                if args.i:
                    dirs_to_make = self.re_filt(args.i, dirs_to_make)
                if dirs_to_make:
                    print(f"dsync: making new dirs ({len(dirs_to_make)}):")
                ...
                if args.rf:
                    ...
                    dirs_to_delete = [
                        ldir for ldir in dir_match if ldir not in dev_dir_match
                    ]
                    dirs_to_delete = [
                        dir
                        for dir in dirs_to_delete
            ...
            if dev_files:
                ...
                if local_files:
                    ...
                    files_to_sync = [(fts[1], fts[0]) for fts in dev_files]
                ...
                if files_to_sync:
                    ...
                    _new_files = [
                        (sz, name)
                        for sz, name in files_to_sync
                        if name not in local_files_dict.keys()
                    ]
                    _modified_files = [
                        (sz, name)
                        for sz, name in files_to_sync
                        if name in local_files_dict.keys()
                    ]
                    if _new_files:
                ...
                if args.rf:
                    ...
                    files_to_delete = [
                        dfile[0] for dfile in local_files if dfile[0] not in _dev_files
                    ]
                    # filter and get only files to avoid trying
                    # to delete files whose parent tree was already deleted
                    files_to_delete = [
