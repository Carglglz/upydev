...
dict_arg_options = {
    ...
    "timeit": ["t", "p", "wss", "f", "s"],
    "stream_test": ["t", "p", "wss", "chunk_tx", "chunk_rx", "total_size"],
    "sysctl": ["t", "p", "wss", "fre", "s"],
    "log": ["t", "p", "wss", "f", "s", "daemon", "follow", "dslev", "dflev", "stopd"],
...
- name: Example playbook
  ...
  tasks:
    ...
    - name: Test test_sh.py parallel
      ...
      """,
    subcmd=dict(help=("indicate a task file to play."), metavar="task", nargs="*"),
    options={
        "-t": dict(help="device target address", required=True),
        "-p": dict(help="device password or baudrate", required=True),
        "-wss": dict(
...
PYTEST = dict(
    ...
    subcmd=dict(
        help="indicate a test script or yaml test file to run, "
    ...
    options={
        ...
        "--yf": dict(help="yaml test file", required=False),
    },
)
...
def timeit_script(args, script):
    ...
    reload = f"del(sys.modules['{script_name}'])"
    timeit_cmd = (
        f"t_0 = tzero();import {script_name};"
...
def ping_diagnose(ip, rep_file=None):
    ...
    if timeouts >= 3:
        ...
        print(
            "WIRELESS DIAGNOSTICS NOT POSSIBLE, PLEASE RESET THE DEVICE OR PLACE IT NEARER TO THE LOCAL AP"
        )
        return False
    else:
        return True
...
def get_error_log(args):
    ...
    dev.output = None
    dev.cmd(
        "from upysh import cat;import uos,gc;uos.listdir('{}')".format(source),
        silent=True,
    )
...
def do_pg_bar(
    index, wheel, nb_of_total, speed, time_e, loop_l, percentage, size_bar=bar_size
):
    ...
    print(
        "▏{}▏{:>2}{:>5} % | DATA: {} | SPEED: {:>5} MB/s | TIME: {} s".format(
            "█" * index + l_bloc + " " * ((bar_size + 1) - len("█" * index + l_bloc)),
            wheel[index % 4],
            int((percentage) * 100),
            nb_of_total,
            speed,
...
def stream_test(args, dev, mode="download"):
    if mode == "download":
        ...
        dev.cmd_nb(
            "w_stream_writer('{}', 8005, {}, {})".format(
                local_ip, args.chunk_tx * _kB, args.total_size * _MB
            ),
            follow=False,
            block_dev=False,
        )
...
def sysctl(args, script):
    ...
    if mode == "start":
        ...
        reload_cmd = (
            f"import sys,gc;{reload};gc.collect();" "print('systctl: script unloaded')"
        )
        time.sleep(1)
        dev.cmd(reload_cmd)
        print("Done!")
    ...
    if not args.daemon:
        if not args.stopd:
            ...
            fh_err = logging.FileHandler(
                os.path.join(filelog_path, "{}_error.log".format(script_name))
            )
...
def follow_daemon_log(args, script):
    ...
    follow_tail = subprocess.Popen(
        ["tail", "-F", filelog_daemon], stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    while True:
        try:
            ...
            print(
                f"Unfollowing, do '$ upydev log {script_name} "
    ...
    if command == "probe":
        if args.gg or args.G:
            ...
            if args.gg and group_file not in os.listdir(UPYDEV_PATH):
                ...
                for key in group_devs.keys():
                    ...
                    if is_reachable:
                        print(
                            "{} {:10} -> {:} @ {:} -> {} {}".format(
                                tree, key, dev_type, dev_add, OK, CHECK
                            )
                        )
                    else:
                        print(
                            "{} {:10} -> {:} @ {:} -> {} {}".format(
                                tree, key, dev_type, dev_add, FAIL, XF
                            )
                        )
        else:
            # check if device in ZeroTier group.
            ...
            if is_reachable:
                print(
                    "{:10} -> {:} @ {:} -> {} {}".format(
                        dev_name, dt, args.t, OK, CHECK
                    )
                )
            else:
                print(
                    "{:10} -> {:} @ {:} -> {} {}".format(dev_name, dt, args.t, FAIL, XF)
                )

    elif command == "scan":
        if args.sr or args.s:
            print("Serial Scan:")
            ...
            if len(devs) == 0:
                ...
                for dev in devs:
                    ...
                    print(
                        "┃{0:^20} ┃ {1:^40} ┃ {2:^10} ┃ {3:^40} ┃".format(
                            dev.name, dev.address, int(dev.rssi), ",".join(services)
                        )
                    )
                    if dev != devs[-1]:
                        print(
        ...
        if args.subcmd[0] in ["add", "rm", "list"]:
            ...
            if play_action == "add":
                ...
                tasks_names = [
                    tsk.replace(".yaml", "") for tsk in _tasks if tsk.endswith(".yaml")
                ]
